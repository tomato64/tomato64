...
#include <stdlib.h>
#include <unistd.h>
/* Tomato64 local addition (differs from upstream Gargoyle): this header uses
 * uint32_t/uint64_t and the .c uses INT32_MAX without ever including
 * <stdint.h>. musl happens to pull it in transitively so the target build
 * works by luck; make the dependency explicit. */
#include <stdint.h>
#include <sys/types.h>
