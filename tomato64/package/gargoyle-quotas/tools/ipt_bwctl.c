					...
					)
{
	...
	*(ip + 3) = *( (uint32_t*)(in_buffer + (*in_index) + 16) );
    /* Tomato64 local fix (differs from upstream Gargoyle): the cast named
     * ip_bw_kernel_data (the response header) while the variable is, and is
     * only ever used as, an ip_bw_kernel_data_item (a per-IP entry) - see the
     * num_nodes/first_start/ipbw_data accesses below, which the header struct
     * does not have. GCC 14+ rejects the mismatched cast outright. */
...
ip_bw* load_usage_from_file(char* in_file_path, unsigned long* num_ips, time_t* last_backup)
{
	...
	*last_backup = 0;
	...
	if(in_file != NULL)
	{
		...
		/* Tomato64 local addition: a zero-length backup file (a save that was
		 * interrupted by a power cut, say) yields no data here; splitting NULL
		 * would fault. Treat it as "nothing saved yet". */
		if(file_data == NULL)
		{
			return NULL;
		}
		...
		*num_ips = 0;
		...
		while(data_part_index < num_data_parts)
		{
			...
			{
				/* Tomato64 local fix: "%ld" writes exactly a long, but time_t
				 * is not a long everywhere - 32-bit musl (bcm53xx) has a
				 * 64-bit time_t with a 32-bit long, so the upper half would be
				 * left as garbage. Scan a long long and narrow deliberately. */
				long long last_backup_tmp = 0;
				sscanf(data_parts[data_part_index], "%lld", &last_backup_tmp);
				*last_backup = (time_t)last_backup_tmp;
				//printf("last_backup = %ld\n", *last_backup);
				data_part_index++;

				/* Tomato64 local fix (differs from upstream Gargoyle):
				 * a backup file holding only the timestamp and no IP data
				 * is completely normal - it is what a quota that nobody has
				 * used yet saves. Upstream falls straight through to the
				 * read below without re-checking the loop bound, walks off
				 * the end of data_parts and segfaults, so every boot after
				 * adding a quota would crash the restore. */
				if(data_part_index >= num_data_parts)
				{
					break;
				}
			}
			/* Tomato64 local fix: family is a uint32_t, but "%ld" makes sscanf
			 * write a full long - 8 bytes into a 4-byte object on any 64-bit
			 * build, smashing whatever sits next to it on the stack. The cast
			 * only hid this from the compiler. */
			sscanf(data_parts[data_part_index], "%u", &family );
			data_part_index++;

			/* Tomato64 local fix: each record is three tokens (family, ip,
			 * bytes) but upstream only bounds-checks at the top of the loop,
			 * so a truncated or corrupt backup file reads past the end of
			 * data_parts. Restore runs on every boot, so a damaged file must
			...
			/* data_part_index bound added by Tomato64: the original test only
			 * bounded the output index, leaving the bytes read below able to
			 * run off the end of a truncated file. */
			if(valid && data_index < num_data_parts && data_part_index < num_data_parts)
			{
				next.family = family;
				if(next.family == AF_INET)
				{
