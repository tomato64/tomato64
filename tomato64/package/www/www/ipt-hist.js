...
function genData() {
	...
	for (i = 0; i < iptHistory.length; ++i) {
		h = iptHistory[i];
		t = getYMD(h[0]);
		d = iptHistoryDaily ?
			[t[0], t[1] + 1, t[2], h[1], h[2], h[3]] :
			[t[0], t[1] + 1, h[1], h[2], h[3]];
		w.document.writeln(d.join(','));
	}
	w.document.writeln('<\/pre>');
	w.document.close();
}
...
function iptHistoryDateText(ymd) {
	return iptHistoryDaily ?
		ymdText(ymd[0], ymd[1], ymd[2]) :
		ymText(ymd[0], ymd[1]);
}
...
function redraw() {
	...
	if (iptHistory.length > 0) {
		...
		for (i = 0; i < iptHistory.length; ++i) {
			...
			var ymd = getYMD(b[0]);
			d = [iptHistoryDateText(ymd), h, rescale(b[2]), rescale(b[3]), rescale(b[2]+b[3])];
			dg.insertData(-1, d);
		}
		dg.resort();
		dg.recolor();
...
function viewQosDetail(n) {
	cookie.set('qos_filterip', [iptHistory[n][1]], 1);
...
function viewIptDetail(n) {
	cookie.set('ipt_filterip', [iptHistory[n][1]], 1);
	popupWindow('ipt-details.asp');
}

function addExcludeList(n) {
	if (E('_f_filter_ipe').value.length<6) {
		E('_f_filter_ipe').value = iptHistory[n][1];
	}
	else {
		if (E('_f_filter_ipe').value.indexOf(iptHistory[n][1]) < 0) {
			E('_f_filter_ipe').value = E('_f_filter_ipe').value + ',' + iptHistory[n][1];
		}
	}
	dofilter();
}
...
function init_filter_dates() {
	...
	if (iptHistory.length > 0) {
		for (var i = 0; i < iptHistory.length; ++i)
			dates.push('0x' + iptHistory[i][0].toString(16));
	...
	for (var i = 0; i < dates.length; ++i) {
		...
		var selected = ((ymd[0] == d.getFullYear()) && (ymd[1] == d.getMonth()) &&
			(!iptHistoryDaily || (ymd[2] == d.getDate())));
		var text = iptHistoryDateText(ymd);

		b.options[i+1] = new Option(text, dates[i], false, selected);
		e.options[i+1] = new Option(text, dates[i], false, selected);
	}
}
...
function update_filter_dates(b) {
	for (var i = 0; i < b.options.length; ++i) {
		if(b.options[i].value.toString() != '0') {
			...
			b.options[i].text = iptHistoryDateText(ymd);
		}
	}
}
...
function writeIptHistoryOptions() {
	var c = [];
	var dateFormats = iptHistoryDaily ?
		[['0', 'yyyy-mm-dd'], ['1', 'mm-dd-yyyy'], ['2', 'mmm dd, yyyy'], ['3', 'dd.mm.yyyy']] :
	...
	c.push({ title: 'Date Format', name: 'f_dafm', type: 'select', options: dateFormats });
