 ...
 * upstream declares an anonymous union here that is structurally identical to
 * union xt_bandwidth_ipany but is a *distinct* type to the compiler, so passing
 * &info->local_subnet to parse_sub() is a pointer-type mismatch. GCC 14 makes
 * -Wincompatible-pointer-types an error by default, so alias the header's union
 * instead. Layout is unchanged. */
