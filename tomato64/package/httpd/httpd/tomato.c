...
#define WIFI_PHY_BLOCK(p) \
	...
	{"wifi_phy" #p "_mode",		V_TEXT(1, 6)			}, /* n, ac, ax, be, legacy */ \
	...
	{"wifi_phy" #p "_ifaces",	V_RANGE(0, 16)			}, \
	{"wifi_phy" #p "_custom",	V_NONE				}, /* Custom hostapd device config */

/* WiFi per-interface variables (phy p, iface i) */
#define WIFI_IFACE_BLOCK(p, i) \
	{"wifi_phy" #p "iface" #i "_enable",		V_RANGE(0, 1)	}, \
	{"wifi_phy" #p "iface" #i "_mode",		V_TEXT(2, 7)	}, /* Access Point, Client, etc */ \
	{"wifi_phy" #p "iface" #i "_essid",		V_TEXT(0, 32)	}, \
	{"wifi_phy" #p "iface" #i "_bssid",		V_TEXT(0, 17)	}, \
	{"wifi_phy" #p "iface" #i "_network",		V_TEXT(0, 3)	}, /* br0 - br7, none "" */ \
	{"wifi_phy" #p "iface" #i "_hidden",		V_NONE		}, /* Hide SSID */ \
	{"wifi_phy" #p "iface" #i "_wmm",		V_RANGE(0, 1)	}, \
	{"wifi_phy" #p "iface" #i "_uapsd",		V_RANGE(0, 1)	}, \
	{"wifi_phy" #p "iface" #i "_encryption",	V_NONE		}, \
	{"wifi_phy" #p "iface" #i "_cipher",		V_NONE		}, \
	{"wifi_phy" #p "iface" #i "_key",		V_LENGTH(8, 64)	}, \
	{"wifi_phy" #p "iface" #i "_isolate",		V_NONE		}, \
	{"wifi_phy" #p "iface" #i "_br_isolate",	V_NONE		}, \
...
#define WIFI_PHY_IFACES(p) \
	WIFI_IFACE_BLOCK(p, 0) \
	WIFI_IFACE_BLOCK(p, 1) \
	WIFI_IFACE_BLOCK(p, 2) \
	WIFI_IFACE_BLOCK(p, 3) \
	WIFI_IFACE_BLOCK(p, 4) \
	WIFI_IFACE_BLOCK(p, 5) \
	WIFI_IFACE_BLOCK(p, 6) \
	WIFI_IFACE_BLOCK(p, 7) \
	WIFI_IFACE_BLOCK(p, 8) \
	WIFI_IFACE_BLOCK(p, 9) \
	WIFI_IFACE_BLOCK(p, 10) \
	WIFI_IFACE_BLOCK(p, 11) \
	WIFI_IFACE_BLOCK(p, 12) \
	WIFI_IFACE_BLOCK(p, 13) \
	WIFI_IFACE_BLOCK(p, 14) \
	WIFI_IFACE_BLOCK(p, 15)

/* Complete block: PHY config + all ifaces */
#define WIFI_BLOCK(p) \
	WIFI_PHY_BLOCK(p) \
	WIFI_PHY_IFACES(p)
