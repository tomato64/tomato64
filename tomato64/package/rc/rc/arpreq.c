...
#define LOGMSG_NVDEBUG		"arpreq_debug"
...
#endif
	logmsg(LOG_DEBUG, "[%s]: send %d (%d) bytes:\n", __FUNCTION__, err, p-buf);
...
#endif
	logmsg(LOG_DEBUG, "[%s]: source:[%s] taget:[%s], dev:[%s]\n", __FUNCTION__, source, target, device);
...
#endif
	strlcpy(buf, inet_ntoa(dst), sizeof(buf)); /* buf needed for inet_ntoa() function! */
	strlcpy(buf2, inet_ntoa(src), sizeof(buf2));
	logmsg(LOG_DEBUG, "[%s]: ARPING to %s from %s via %s\n", __FUNCTION__, buf, buf2, device);
